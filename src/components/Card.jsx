...
export default function Card({ projects, isSwiper, isDelete, getData }) {
  ...
  const handleDelete = (projectId, title) => {
    CheckModal.fire({
      ...
      const response = await axios.get(`${apiBase}/favorites?projectId=${projectId}&userId=${userId}`);
          ...
          <div className="card-body p-3 d-flex flex-column">
            ...
            <div className="d-flex justify-content-between align-items-center mb-4 mt-auto">
              <div className="d-flex align-items-center">
                <span className="material-symbols-outlined text-white fs-7">person</span>
                <p className="text-white mb-0 fs-sm fs-lg-base">{project.supportNum}</p>
              </div>
